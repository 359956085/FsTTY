...
此功能位于开发分支，尚未作为新版本发布。Windows 安装包将桌面与 `fstty-broker.exe` 安装到 `Program Files\FsTTY`，服务名为 `FsTTYBroker`，账号为 `NT SERVICE\FsTTYBroker`。其他平台继续使用进程内 SSH 与系统凭据库。
...
Windows 使用机器范围 NSIS，固定安装目录。安装引导程序从 Program Files 的独立临时目录执行，在检查旧目录权限后停止服务、备份密文数据库和程序。服务启动失败会尝试恢复旧程序及数据库；无法自动恢复时明确报错并保留数据。
