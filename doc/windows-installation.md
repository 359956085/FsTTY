...
安装器在 UAC 前保留普通调用进程；另一管理员批准后仍读取原用户登记，桌面以原用户令牌启动。从已提权终端启动时，安装器先验证调用进程：存在可信的关联普通令牌就用它启动桌面；内置 Administrator 或关闭 UAC 导致没有普通令牌时进入管理员兼容模式，桌面会继续以管理员权限运行。会话 0、SYSTEM、服务账号、跨会话调用、SID 或会话不一致的关联令牌均会被拒绝。
...
交互安装进入管理员兼容模式时只显示一次说明；确认后自动继续。静默安装不新增参数，会直接继续并在日志中标记 `caller_mode=alwaysElevated`。若存在关联普通令牌，日志标记 `linkedStandard`，安装后的桌面恢复普通权限；普通调用者标记 `standard`。

在出现调用者身份或会话错误的机器上，从启动安装器的同一个桌面或终端运行仓库中的只读诊断脚本：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\diagnose-windows-installer.ps1
```

脚本只读取当前令牌、Windows 版本和 UAC 策略，输出 `CallerMode`、`TokenElevationType`、关联令牌状态、`EnableLUA`、`FilterAdministratorToken` 及会话 ID，不修改系统。SID 以 `-500` 结尾表示内置 Administrator；`EnableLUA=0` 表示 UAC 管理员批准模式关闭。

独立安装器日志位于 `C:\ProgramData\FsTTY\logs\installer-<date>-<run-id>.log`。SYSTEM 和 Administrators 可写，普通用户只读，日志保留 15 天。它记录安装版本、安装或更新模式、阶段、会话、调用者模式、目标路径、退出码和回滚结果，不记录完整 SID、代理凭据、签名或安装包内容。应用更新日志继续位于 `%APPDATA%\FsTTY\logs`。

不要手工构造 `/CALLERPID` 或跳过 Broker 的令牌校验。兼容模式只适用于交互式的非 SYSTEM 管理员账号，不能放宽会话、身份、安装路径、签名和事务回滚边界。完整验收仍需在原报错环境或可还原虚拟机中进行。
...
1. 从普通用户桌面启动，由 UAC 完成安装；新装到含中文和空格的目录，运行下方 `AssertInstalled`，并验证普通用户不能改写服务及安装记录。
...
4. 普通管理员从已提权终端启动安装，确认日志为 `linkedStandard`，安装后的桌面恢复普通权限；使用另一管理员批准普通用户发起的 UAC，桌面仍属于原用户。取消 UAC 和旧进程关闭确认时保持旧安装。
5. 分别在内置 Administrator 和关闭 UAC 的快照中测试交互安装、静默安装与在线更新；确认日志为 `alwaysElevated`，交互安装只说明一次，静默安装自动继续，桌面保持管理员权限。
6. 以 SYSTEM、会话 0 和跨会话进程尝试调用 Broker，并伪造 SID 不一致、会话不一致或仍提权的关联令牌；均须在部署前拒绝。
7. 用虚拟机检查点模拟服务安装失败、磁盘写入失败及安装中断；重新运行包后核对旧版本、服务、登记、凭据数据和桌面恢复一致。签名在线升级还需使用匹配发布测试密钥的两个递增版本。
8. 尝试硬链接、目录联接、父目录替换及伪造产品文件；应拒绝替换且不改变目录外文件。检查安装日志 ACL、重解析点和硬链接防护，以及 15 天保留期。
9. 卸载当前安装，执行 `AssertUninstalled`；旧目录、额外文件、用户配置及受保护数据必须保留。
