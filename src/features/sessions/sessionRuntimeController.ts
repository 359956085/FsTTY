...
export function createSessionRuntimeController(): SessionRuntimeController {
  ...
  return {
    ...
    isDevicePollCurrent: (sessionId, pollId) => devicePolls.get(sessionId) === pollId,
    isDeviceRequestCurrent: (sessionId, requestId) =>
      deviceRequests.get(sessionId) === requestId,
    isFileRequestCurrent: (sessionId, requestId) => fileRequests.get(sessionId) === requestId,
    isUploadBatchCurrent: (sessionId, token) => uploadBatches.get(sessionId) === token,
    removeSession,
    ...
    startUploadBatch: (sessionId, token) => {
      if (uploadBatches.has(sessionId)) return false;
